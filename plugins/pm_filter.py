...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, NOR_IMG, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, REQ_CHANNEL, TUTORIAL, REQST_CHANNEL, IS_TUTORIAL, LANGUAGES, SEASONS, SUPPORT_CHAT, PREMIUM_USER
...
async def pm_text(bot, message):
    ...
    k = await message.reply_text(
         ...
         reply_markup=InlineKeyboardMarkup([[
             InlineKeyboardButton("✼ 𐌺ᥱrᥲᥣᥲ R᧐ᥴκᥱrs 𝟹.𝟶​ ✼", url=f"https://t.me/KLMovieGroup")
             ],[
             InlineKeyboardButton("❐ 𐌺ᥱrᥲᥣᥲ R᧐ᥴκᥱrs 𝟸.𝟶​ ❐", url=f"https://t.me/KL_Group2")]]))
...
async def next_page(bot, query):
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                btn.append(
                    [InlineKeyboardButton("⇚ Bᥲᥴκ​", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages")]
                )
            elif off_set is None:
                btn.append([InlineKeyboardButton("⎘ Pᥲgᥱ", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("𐌽ᥱ᥊ᴛ​ ⇛", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⇚ Bᥲᥴκ​", callback_data=f"next_{req}_{key}_{off_set}"),
                        InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} - {math.ceil(total/10)}", callback_data="pages"),
                        InlineKeyboardButton("𐌽ᥱ᥊ᴛ​ ⇛", callback_data=f"next_{req}_{key}_{n_offset}")
                    ],
                )
        else:
            ...
            if n_offset == 0:
                btn.append(
                    [InlineKeyboardButton("⇚ Bᥲᥴκ​", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages")]
                )
            elif off_set is None:
                btn.append([InlineKeyboardButton("⎘ Pᥲgᥱ", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("𐌽ᥱ᥊ᴛ ​⇛", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⇚ Bᥲᥴκ​", callback_data=f"next_{req}_{key}_{off_set}"),
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="Select Your Desired 𐍃ᥱᥲs᧐ᥒs", callback_data=f"fs#unknown#{key}"
            )
        ],
    )
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="⇚ ʙᴀᴄᴋ ᴛᴏ ғɪʟᴇs ⇛", callback_data=f"fs#homepage#{key}")])
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings["button"]:
        ...
        btn.insert(0, [
            ...
            InlineKeyboardButton("𐍃ᥱᥣᥱᥴᴛ Agᥲiᥒ", callback_data=f"seasons#{key}")
        ])
    else:
        btn = []
        btn.insert(0, 
...
#        await query.answer(url=f"https://telegram.me/{temp.U_NAME}?start=file_{file_id}")
        ...
        await query.answer(f"⚡️ sᴇʀᴠᴇʀ sᴛᴀᴛᴜs ⚡️\n\n◉ ᴜᴘᴛɪᴍᴇ : {currentTime}\n◉ ᴄᴘᴜ : {cpu_usage}\n◉ ʀᴀᴍ : {ram_usage}\n◉ ᴛᴏᴛᴀʟ ᴅɪsᴋ : {total}\n◉ ᴜsᴇᴅ sᴘᴀᴄᴇ : {used} ({disk_usage}%)\n◉ ғʀᴇᴇ sᴘᴀᴄᴇ : {free}", show_alert=True)    
        ...
        buttons = [[
            ...
            InlineKeyboardButton('• ʙᴏᴛ sᴇʀᴠᴇʀ ɪɴꜰᴏ •', callback_data='statx')
        ],[
        ...
        if set_type == 'botpm' and query.from_user.id not in ADMINS:
            return await query.answer(text=f"Hᴇʏ {query.from_user.first_name}, Yᴏᴜ ᴄᴀɴ'ᴛ ᴄʜᴀɴɢᴇ sᴇᴛᴛɪɴɢs ғᴏʀ ʏᴏᴜʀ ɢʀᴏᴜᴘ !\n\nIᴛ's ᴀɴ ᴀᴅᴍɪɴ ᴏɴʟʏ sᴇᴛᴛɪɴɢ !", show_alert=True)
        ...
        if set_type == 'file_secure' and query.from_user.id not in ADMINS:
            return await query.answer(text=f"Hᴇʏ {query.from_user.first_name}, Yᴏᴜ ᴄᴀɴ'ᴛ ᴄʜᴀɴɢ sᴇᴛᴛɪɴɢs ғᴏʀ ʏᴏᴜʀ ɢʀᴏᴜᴘ !\n\nIᴛ's ᴀɴ ᴀᴅᴍɪɴ ᴏɴʟʏ sᴇᴛᴛɪɴɢ !", show_alert=True)
        ...
        if set_type == 'is_shortlink' and query.from_user.id not in ADMINS:
            return await query.answer(text=f"Hey {query.from_user.first_name}, You Can't Change Settings For Your Group !\n\nIt's An Admin Only Setting !", show_alert=True)

        if set_type == 'auto_ffilter' and query.from_user.id not in ADMINS:
            return await query.answer(text=f"Hey {query.from_user.first_name}, You Can't Change Settings For Your Group !\n\nIt's An Admin Only Setting !", show_alert=True)

        if set_type == 'max_btn' and query.from_user.id not in ADMINS:
            return await query.answer(text=f"Hey {query.from_user.first_name}, You Can't Change Settings For Your Group !\n\nIt's An Admin Only Setting !", show_alert=True)
